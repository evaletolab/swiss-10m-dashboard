...
export async function normalizeGePublicTransportFinance() {
  ...
  if (!existsSync(fromRoot(manualCsvPath))) {
    addDownloadRequest({
      ...
      missingFields: ['year', 'transport_revenue_million_chf', 'operating_subsidy_million_chf', 'operating_expenses_million_chf'],
      reason: 'Comparer la croissance démographique et économique avec les finances et subventions des transports publics.',
      preferredSource: 'Rapports annuels TPG, états financiers TPG et contrats de prestations',
      ...
      instructions: 'Déposer une série annuelle TPG avec revenus de transport, revenus abonnements, indemnités/subventions d’exploitation et charges d’exploitation.',
    })
